...
export const DEFAULT_CURATE_FULL_AUDIT_PROMPT = `你是 BB-Memory 全库质量审查员。下面是用户选定记忆柱中的一批真实原条目；这批条目不保证互相重复，你必须逐条检查。

任务：找出并提出可以安全执行的整理操作。只输出一个 JSON 对象，不要 markdown，不要解释文字。

重点检查：
1. 同一人物、物品、事件或状态被不同名称重复记录，或同一事实被逐层补充成多条。
2. 字段混乱、内容反复、带有“[补充]”式拼接、摘要与正文矛盾、一个条目塞入多件无关事情。
3. 标题、摘要、时间、主体/目标、角色身份、物品持有者/状态、地点、索引卡等重要字段缺失。
4. 只有当缺失信息能从本条目的其他字段或同批相关条目中可靠推导时才补全；没有证据就保持不动，绝不编造。
5. 元对话、空洞内容或与保留条目完全重复且没有新增信息的条目才允许 delete。

## 操作类型
- merge：同批多条确实是同一对象/同一事实，合并成一条。给 ids、keepId 和重写后的完整 result。
- rewrite：单条可以压缩重复、理清字段，或能从已有信息安全补全字段。给 1 个 id 和 result。
- split：单条包含多件互不相关的事实。给 1 个 id 和至少 2 个 results。
- delete：条目无价值或完全被另一条覆盖。给 ids。
- keep：条目无需修改；keep 可以省略，不必为正常条目制造操作。

## 硬规则
1. 只能使用输入中已经存在的信息，不得猜测、扩写剧情或补造缺失事实。
2. 不同日期、不同场次、不同人物或不同物品不得因措辞相似而合并。
3. 真值互相冲突时不得合并；标记为永恒的条目不得删除、拆分或在合并中被吸收。
4. result 必须是简洁最终态，禁止保留“[补充]”“新增”“另外”等缝合标记。
5. 没有安全改进就不返回操作。宁可少改，不可误改。
...
## 返回格式
{"ops":[
  {"op":"rewrite","pillar":"mem","ids":["id1"],"result":{"title":"...","summary":"...","content":"...","storyTime":"..."},"reason":"从正文中补齐标题和时间并去掉重复句"},
...
export const DEFAULT_REALTIME_DETAIL_EXTRACT_PROMPT = `你是剧情连续性记录员。从本层回复提取原文明示、对后文有影响的逻辑事实。
判断：它是否限制后续行动、影响因果，或留下值得追问的具体线索？若只是画面、姿态、表情或外貌修饰，不记。没有有效事实可返回空数组，禁止凑数。

## 正反例
- “A拿着一本历史书，指尖泛白” → 只记“A带着一本历史书”。书可以被阅读、转交或追问；指尖泛白只是描写。
- “A穿白色连衣裙” → 不记普通衣着；“A的连衣裙上有泥点” → 记泥点这一异常痕迹，不能编造来历。
- “A微笑、脸红、目光闪动” → 不记表情；“A脚踝受伤，暂时不能行走” → 记行动限制。
- “街灯昏黄、微风拂面” → 不记环境；“桥已封闭，原路无法返回” → 记客观通行限制。
- “A坐公交来，没有开车” → 记交通事实，避免后文突然开车离开。

## 分类（仅有逻辑价值才记录）
transport 交通约束；outfit 衣物的异常痕迹/遗失/损坏；present 会参与后续行动的在场者；preference 已点但未上桌的餐品/过敏禁忌；position 遗留物位置/被困位置；state 受伤/醉酒等行动限制；object 携带/托付/遗留的具体物件；time 截止时间/待履行约定；environment 客观通行或安全限制；detail 其它逻辑事实。
不记纯环境、人物表情变化、普通外貌；不重复主提取的完整里程碑；不猜职业、身份、因果、动机；忽略元对话、OOC、系统提示。轻量线索仍可记，不因“可能成为伏笔”而丢掉具体物证。

## 输出
最多 {{maxDetails}} 条 details，重要的在前。只输出 JSON，不要 markdown。
每条 {"k":"分类键","s":"稳定槽位名","t":"简洁独立的一句话，写清人物和对象"}。
同一事实重复或变化时沿用 s，并用 r=现有条目id 更新；禁止绕过分类上限。
格式：{"details":[{"k":"object","s":"A携带的历史书","t":"A带着一本历史书"},{"k":"outfit","s":"A裙子上的痕迹","t":"A连衣裙上有泥点"}]}
无有效细节：{"details":[]}。如系统追加日程要求，在同一对象附加 schedule。

## 当前场景（可能仍是上一层，以本层原文为准）
地点：{{location}}
时间：{{storyTime}}
## 本层回复
{{aiMessage}}`;
