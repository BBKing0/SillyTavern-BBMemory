...
读取边界：
- 角色卡和世界书通常是背景设定，优先提取 NPC、物品、地点、世界观事实、持续时间线线程。
- 聊天记录中已经发生的剧情可以提取为记忆条目和时间线事件。
- 不要把 OOC、元指令、工具说明当作剧情记忆。
- 不确定的信息可以用 truthStatus:"unknown" 或线程 status:"paused" 标记。
...
6. threads 数组：{ "n":"线程名", "tp":"plot/emotional/side/world", "st":"ongoing/paused/ended/resident", "p":"high/medium/low", "s":"一句话总结", "entries":[] }

返回 JSON：{"memories":[],"npc":[],"items":[],"timeline":[],"locations":[],"threads":[]}
...
你能读取并解释：记忆、NPC、物品、时间线、地图地点、线索板节点。用户只是询问或列举时，直接基于数据快照回答，不要编造。
...
export const DEFAULT_THREAD_SUMMARY_PROMPT = `你是一个故事时间线组织助手。根据时间线条目和已有线程，重新整理故事线程。{{calRef}}
...
规则：
1. 每个线程有独立的 name，例如“第一年·战前”“感情线·CharA”“支线·寻找圣剑”。
2. 将相关的时间线条目归入对应线程的 entries 中。
3. 合并同类项，时间相近、主题相同的事件合并为一条 entry。
4. 保持活跃线程在 {{maxActive}} 条以内，resident 不计入。
5. 已结束的线程标记 status:"ended"。
6. 重要的、贯穿始终的线程标记 status:"resident"。
7. 线程类型 type: plot / emotional / side / world。

返回纯 JSON 对象（不要 markdown 代码块）：
{"threads":[{"id":"保留已有ID或生成新ID","name":"线程名","type":"plot|emotional|side|world","status":"ongoing|ended|paused|resident","priority":"high|medium|low","parentThreadId":null,"entries":[{"period":"具体时间区间","event":"事件描述","status":"ongoing|ended|milestone"}]}]}
只输出 JSON。`;
