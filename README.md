...
- **初始化工作台** — 读取当前角色卡/世界书/聊天记录，或上传 JSON/TXT，由 LLM 提取可审阅草稿
...
- **初始化与合并** — 选择提取记忆、NPC、物品、地点、时间线、时间线程，支持多波次草稿合并
- **手动管理** — 添加、编辑、删除、搜索记忆
- **导入/导出** — JSON 格式备份和恢复
...
1. 点击侧边栏的"初始化记忆"按钮，打开初始化工作台。
2. 选择"读取当前酒馆资料"，或上传角色卡、世界书、聊天记录、已有草稿 JSON。
3. 勾选本轮要提取的类型：记忆、NPC、物品、地点、时间线、时间线程。可以第一轮只提记忆，第二轮只提地图，然后合并到同一份草稿。
4. AI 提取后先在草稿区检查，可按类型编辑、删除、新增，再保存到当前聊天的 BB-Memory 数据库。
5. 保存时会按名称/事件/地点合并已有条目，记忆条目会优先使用向量去重；未启用向量时使用标题与内容精确键去重。

### 外置初始化提取工具

1. 打开 `tools/bbmemory-initializer.html`，上传角色卡、世界书、聊天记录，或上传已有 BB-Memory JSON/草稿 JSON 继续合并。
2. 勾选本轮要提取的类型：记忆、NPC、物品、地点、时间线、时间线程。工具支持多波次合并，例如第一轮只提记忆，第二轮只提地点和物品。
3. 世界书 JSON 会解析成可开关条目；关闭不希望 AI 读取的条目后，再进行提取或分段。
4. 可以先用"记忆世界书分段"让 AI 按 5 天/10 天/自然时间段切分，检查分段后再并行提取。
5. "跨世界观转换"支持上传原世界观、记忆存档、新世界观，按 10/20 条记忆一组转换，再总结 NPC、物品、地点、时间线和时间线程。
6. 配置 OpenAI 兼容 API 后运行提取。外置工具不会自动读取酒馆内容，也不会直接写入 IndexedDB。
7. 提取结果会进入草稿池，可按类型搜索、编辑、复制、删除、新增，并可保存到浏览器本地草稿。外置工具导出的记忆和地图等级最低为 `stable`，AI 可升格为 `core` / `eternal`。
8. 最终点击"下载导入 JSON"生成 BB-Memory 可导入文件；地点会同时写入 `map.locations`，用于恢复地图节点和连接。
