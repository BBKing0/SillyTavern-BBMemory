...
> **v9.4.6：重做记忆管家与同类问题一键维护**
>
> - 管家使用副 API，多轮搜索当前聊天全库并读取完整原文，给出修改/维护建议。覆盖五柱、时间线、地图、线索节点/连线、所有分类、归档和隐藏备注；不包含其他聊天或未加载存档。
> - 建议显示真实 ID、理由和字段修改前后内容；勾选后点击“执行所选建议”，或输入“执行建议 1、3”“执行全部建议”。模型不会自动写入；并发修改会拒绝覆盖。
> - “整理维护建议”综合体检与待维护项，可提出补向量、忽略、保留、升降级、归档及时间线修正等建议，按用户选择执行。
> - 维护→体检：同类问题“一键忽略”；缺少向量和语义孤立“一键重新生成向量”。缺失检查覆盖记忆、NPC、物品、里程碑、时间线和地图，实时记忆不需要向量。
> - 维护→待维护：同类一键忽略/保留，积灰物品一键归档/升稳定，已结束里程碑一键压缩。显示进度和实际成功/失败数量，失败项保留供重试。
> - 忽略按聊天保存并通过聊天元数据同步，内容变化后重新提示；体检页可恢复忽略。“重置对话”只清空管家对话和建议，不清空记忆。

管家调查使用副 API，补向量使用 Embedding API。设置页可调整调查轮数、分页条数、原文分段字符数、历史消息数、超时及输出 token 上限。大库达到轮数上限会明确提示，可继续追问或调高轮数。本地扫描全库不代表模型已经逐条核验整库。

测试步骤：更新并刷新 ST 后确认 v9.4.6；输入“林澈不是医生，是老师，请核查”，展开建议检查 ID、原值、新值，未执行前原文不变，勾选执行后搜索显示新值。然后在维护→体检对缺少向量一键补全/忽略，再重新体检确认。边界检查：建议后手动改同字段应拒绝旧建议覆盖；Embedding API 部分失败应保留失败项并显示具体原因。

开发回归：`node BB-Memory/tests/v9.4.6-regression.mjs` 和 `node BB-Memory/tests/v9.4.5-regression.mjs`。环境已有 Playwright/Chromium 时运行 `node BB-Memory/tests/v9.4.6-browser.mjs`，覆盖1280px和390px；可用 `BB_PLAYWRIGHT_MODULE` 指向已有 Playwright 的 package.json，用 `BB_BROWSER_CHANNEL=msedge` 选择 Edge。服务仅绑定127.0.0.1，API与数据均模拟，截图保存到工作区 `_tmp_v946_screens/`。Node/Playwright仅用于测试，扩展运行不依赖它们。

> **v9.4.5：每日日程、逻辑细节与记忆纠错**
>
> - 换楼刷新覆盖实时细节、日程和撤销快照，来源显示“旧聊天”。
> - 日程按故事日保留简短行动轨迹，跨地点不清空；默认独立注入最近3天。日期未知时不猜年月日，历史行程可在实时页“补记日程”。
> - 提示词保留历史书、裙上泥点，排除指尖泛白、普通衣着及环境渲染，并禁止推断身份或因果。
> - **记忆管理 → 纠错**：关键词/职业同义词、中文片段、人物别名和引用关联取并集，覆盖五柱、时间线、地图、线索板、所有分类、归档及隐藏备注，全部结果分页显示。
> - 逐字段修改原条目；候选供人工核对。修改职业时也请检查索引卡、关系和摘要。保存会清除旧向量，可用现有补全向量功能重建。

日程自动记录从后续自动提取开始。日程开启时，细节只抓前N层的设置不会停止后续楼层的日程调用。所有新增日程和纠错参数均可在设置页调整；纠错搜索不消耗API。

开发回归：从仓库根目录运行 `node BB-Memory/tests/v9.4.5-regression.mjs`。测试使用内存存储与模拟API，不访问真实聊天；Node仅用于测试。
> **v9.3.3：AI 记忆整理师 + 实时记忆第五柱**
>
> - AI 整理师会用松阈值召回和链式聚类把“逐步补充、两两不够像”的重复条目放进同一组，再让 AI 重写成最终态；合并、重写、拆分、删除分别支持自动、提醒、确认授权，并保留撤销快照。
> - 新增实时记忆 `bb_rt_chat_<chatId>`，保存交通、衣着、在场者、随手物位置等短期场景细节。它绕过关键词/向量检索，无条件注入，同时受独立条数和 token 双硬上限保护。
> - 实时细节由 TTL、场景切换、容量或手动操作触发结算。`discard` 只停止注入并留档，不立即删除；已结算留档超过容量两倍后才机械修剪最旧的未晋升条目。
...
1. 进入任意角色聊天。
2. 打开右侧扩展设置，展开 **BB-Memory v9.3.3**。
...
| 8 | 主提取同时会对该层 AI 回复做一次轻量场景细节抓取；失败不会影响主提取标记 | 实时记忆、抓取范围 |
| 9 | 新增条目达到阈值后，AI 整理师在后台聚类并按授权矩阵处理重复组 | AI 记忆整理 |
| 10 | 成功处理若干 exchange 后，如果启用时间线总结，会在后台尝试刷新时间线 | 启用时间线总结 |
...
| 渐进细化型重复 | AI 整理师以较松阈值建边并做链式聚类，让一次整理看到整组条目后重写为最终态，而不是逐条拼接 |
...
| 2 | 加载 NPC、物品、里程碑、时间线、记忆、实时记忆、地图和线索板 | 实时记忆、线索板注入、语义向量 |
...
| 7 | 按 Token 预算裁剪长期内容，核心、永恒、常驻内容优先保留 | Token 预算、Token 预算模式 |
| 8 | 把仍生效/待结算的实时细节无条件追加；它不占长期库预算，但受独立条数和 token 上限保护 | 实时注入条数上限、实时 token 上限 |
| 9 | 用注入模板包裹结果，通过 `setExtensionPrompt` 写入 SillyTavern 生成上下文 | 注入模板 |
| 10 | 对实际注入的长期条目记录命中；未命中的活跃条目可能累积未命中计数并触发升降格 | 升级/降级阈值、多样性限制 |
...
| 启用 AI 记忆整理 | 是否在各柱新增计数达标后聚类整理 | 关闭后仍保留增量去重和手动全库整理 |
| 启用实时记忆 | 是否抓取并无条件注入当前场景细节 | 关闭后第五柱不抓取、不注入、不自动结算，已有数据保留 |
...
| 实时记忆 | 交通、衣着、座位、在场者、临时偏好和物品位置 | 绕过检索无条件注入；结算后留档或晋升长期库 |
...
- **实时**：浏览和编辑第五柱，手动晋升、留档/恢复、立即结算和撤销结算。
...
整理师不是只拿一条新记忆和一条旧记忆比较。它会从本批新增条目出发召回同柱候选，以默认 `0.65` 的松阈值建边，把相邻相似但首尾差异较大的渐进细化内容连成一组，再把完整组交给 AI 输出最终态。

- 自动整理按 NPC、物品、里程碑、时间线和记忆各自的新增计数触发。
- “全库整理”会分块扫描所有长期柱，建议先审核再写入。
- 合并、重写、拆分、删除分别可设为直接执行、执行并提醒或必须确认。
- 每次写入前保存撤销快照；侧边栏可点“撤销上次整理”。
- 永恒记忆禁止删除和拆分；真值冲突不得合并。

### 实时记忆

第五柱负责容易被检索遗漏、但当前场景必须连续的细节，例如“坐公交车来的”“谁穿什么”“东西随手放在哪里”。它不生成 embedding，也不等待用户消息命中关键词；只要仍处于生效或待结算状态，就会进入 `【当前场景细节】` 区块。

- 抓取与主提取并行，副调用失败不会让主提取失败或重复标记楼层。
- 可选择每层抓取，或只抓每个场景前 N 层。
- TTL、场景切换、容量上限和“立即结算”都能把条目推进结算。
- AI 可选择晋升到记忆、NPC、物品或里程碑，也可延期或留档。
- 留档只停止注入，不立即删除；超出 `realtimeMaxEntries × 2` 后才修剪最旧的未晋升留档。
- 完整管理器的“实时”页支持编辑、手动晋升、留档/恢复、立即结算和撤销。
...
| 设置 | 代表什么 | 建议 |
| --- | --- | --- |
| 启用 / 触发模式 | 任一柱、全部柱达标或仅手动时整理 | 默认任一柱达标 |
| 各柱触发阈值 | 新增多少条后进入一次后台整理 | 高频柱可调高 |
| 每条召回数 / 聚类阈值 | 候选范围与链式建边松紧 | 默认 5 / 0.65；不确定时别提高阈值 |
| 单次最大组数 | 限制一次 AI 整理的规模 | 默认 8 |
| 合并 / 重写 / 拆分 / 删除授权 | 直接执行、执行并提醒或必须确认 | 默认合并/重写提醒，拆分/删除确认 |
| 撤销深度 | 保留最近几次整理快照 | 默认 3 |
| 时间冲突粒度 | 按日期、精确时间或关闭惩罚 | 默认按日期；同日时刻差异不扣分 |

### 实时记忆

| 设置 | 代表什么 | 建议 |
| --- | --- | --- |
| 启用实时记忆 / 启用抓取 | 总开关与是否产生新细节 | 默认开启 |
| 抓取范围 / 场景前 N 层 | 每层抓取或只抓每场景开头 | 想省 API 可用前 N 层 |
| 单层最多细节 | 一层最多保存几条 | 默认 5 |
| 注入条数 / token 上限 | 不依赖长期库预算的双硬上限 | 默认 15 / 300，避免无声膨胀 |
| TTL / 生效容量 / 场景切换 | 自动推进待结算的三类触发 | 默认 12 层 / 40 条 / 开启 |
| 结算时机 | 自动调用 API 或只在手动时结算 | 默认自动 |
| 晋升写入方式 | 直接写长期库或写前确认 | 风险敏感时选确认 |
...
| `/bb-stats` | 显示 NPC、物品、里程碑、时间线、记忆和实时记忆统计 | toast 显示 |
...
| `/bb-agent` | 打开记忆管家 | 全库核查、建议预览、选择执行 |
...
memory-curator.js         跨条目聚类整理、授权矩阵、审核与撤销
realtime-memory.js        第五柱轻量抓取、结算、晋升与撤销
...
map-store.js              地图数据存储
map-view.js               世界地图 UI
memory-agent.js           记忆管家面板（调查、建议与执行）
...
localforage: bb_rt_chat_<chatId>        实时场景细节（不存向量）
...
- 新增 AI 记忆整理师：从新增条目向量/文本召回候选，以松阈值链式聚类渐进细化型重复，再让 AI 重写为最终态。
- 整理操作按 merge/rewrite/split/delete 分别配置授权；高风险操作进确认面板，写入前保存可撤销快照。
- 时间冲突惩罚默认改为按日期判断，同一天内的 `12:01` 与 `12:05` 不再额外扣分，跨日期仍保持惩罚。
- 新增实时记忆第五柱：与主提取并行抓取场景细节，不做 embedding，并以独立条数/token 双硬上限无条件注入。
- 新增 TTL、场景切换、容量和手动结算；无长期价值条目只留档不立即删除，已晋升条目保留溯源链。
- 第五柱接入管理器、助手、悬浮球、存档槽、JSON 导入导出、chatMetadata 备份、清空和统计。
